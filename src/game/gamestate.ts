...
export class GameState {
    ...
    get legalMoveIndices(): number[] {
        ...
        if (ledSuit === null) {
            ...
            if (legalCards.length === 0) {
                ...
                legalCards = hand;
            } else if (this.config.trickplay === 'allfours') {
                // if we do have cards of led suit, 
                // in All Fours Calypso we may also play a trump
    ...
    getStateForUI(): GameStateForUI {
        return ({
            ...
            trumps: Object.fromEntries(
                this.players.map(
                    (player) => [player.name, player.trumpSuit]
               )
